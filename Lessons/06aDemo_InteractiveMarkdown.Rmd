...
```{r Create base, echo=FALSE, eval=FALSE, warning=FALSE}
#Construct the plot
...
my_plot <- nutrient_data %>% 
  ...
  scale_shape_manual(values = c(17, 15)) +
  labs(
    title = paste("Concentration of",the_parameter,"over time, by depth and lake"),
    x = "Date", 
    y = expression(Concentration ~ (mu*g / L)), 
    shape = "Lake", 
    color = "Depth (m)") +
...
#create a slider widget to set the start and end
renderPlot(
  plot_it(
    input$the_nutrient,
    input$yearSlider[1],
...
renderText(
  paste("you selected",input$the_nutrient)
)
