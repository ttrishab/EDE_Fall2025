...
#Create an output directory (to hold saved plots) <--NEW
...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    label="paste('Mean is ', mu,',', R^{2},'=0.06')",
    parse = TRUE
    )
...
#Save the plot
ggsave(
  filename = here("./Output/Plot_text.jpg"),
  plot = Plot_text, 
  height = 4, 
  width = 6, 
  units = "in", 
...
EPAair %>%
  ggplot(aes(x=PM2.5, y=Ozone, color =mean_Lat))+
