...
stat_summary(EPAair)
Ozone.boxplot <- ggplot(EPAair, aes(x = as.factor(Year),
                                    y = Ozone)) +
  geom_boxplot()  +
...
Ozone.violin <- ggplot(EPAair, aes(x = Year, y = Ozone)) +
  geom_violin()
...
TPAIC <- lm(data = EPAair, PM2.5 ~ Ozone +Year +Month +mean_Lat,mean_Lng)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
TPmodel <- lm(data = EPAair, PM2.5 ~ Ozone +Year +Month +mean_Lat,mean_Lng)
summary(TPmodel)
...
Trimmodel <- lm(data = EPAair, PM2.5 ~ Ozone +Month +mean_Lat,mean_Lng)
summary(Trimmodel)
```
