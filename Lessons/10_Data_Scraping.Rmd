...
> For discussion: Can you give examples of ethical and unethical web scraping? terms of service- view only no scrape, /robots.txt after url if wirecutter/csv then no scraping. api or link, denial of service attack (repetative gettign data can look like bot attack)
...
#could also Month = rep(1:12)
df_withdrawals <- data.frame("Month" = c(1,7,2,8,3,9,4,10,5,11,6,12), #<--NOTE: Different than recording!!
                             "Year" = rep(2020,12),
...
#Plot, because it's fun and rewarding (FIXED SINCE RECORDING)
...
the_df <- expand.grid(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% 
  bind_rows()
...
```{r}
#Select the last three facilities
the_facilities <- tail(the_facility_ids,3)
#Create a vector of years
the_years  <-  c(2018,2019,2020)

#Create a "grid" of input parameters
params <- expand.grid(the_year = the_years, the_facility = the_facilities)

#Mapp
the_df <- pmap(params, scrape.it) %>% bind_rows()
