...
#first and last observation, create sequence with appropriate time steps and then convert to data frame
...
#if month not in months then when join the na show up after join
...
BR_rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901,1), frequency = 12)
...
BR_rainfall_decomposed <- stl(BR_rainfall_ts, s.window = "periodic")
plot(BR_rainfall_decomposed)
...
BR_rainfall_Components <- as.data.frame(BR_rainfall_decomposed$time.series[,1:3])
...
BR_temp_trend <- Kendall::SeasonalMannKendall(BR_temp_ts)
BR_temp_trend
summary(BR_temp_trend)

BR_rainfall_trend <- Kendall::SeasonalMannKendall(BR_rainfall_ts)
BR_rainfall_trend
summary(BR_rainfall_trend)
