...
#Get the data, which starts on line 27
gage_data <- read.table(
  theURL,
  skip = 26, #Skip non-data lines
  header=TRUE,
  sep='\t',
  stringsAsFactors = T) 
...
#Plot
ggplot(gage_data2,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
```
...
the_website <- "https://www.eia.gov/electricity/state/"

#2&3 Locate elements and read their text attributes into variables
the_states <- the_website %>% html_nodes () %>% html_text
the_price <- the_website %>% html_nodes () %>% html_text
the_capacity <- the_website %>% html_nodes () %>% html_text
net_generation <- the_website %>% html_nodes () %>% html_text
total_retail <- the_website %>% html_nodes () %>% html_text
