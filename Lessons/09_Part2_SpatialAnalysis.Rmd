...
triCo_intersect <- huc8_sf_utm %>%  
  st_filter(triCo, .predicate = st_intersects)

myMap + mapview(triCo_intersect, alpha.regions = 0, color='red')

#Intersect the HUC_8s
triCo_counties_sf <- triCo %>% st_intersection(huc8_sf_utm)

myMap + mapview(triCo_counties_sf, alpha.regions = 0)
...
#Combine the centroids into one feature and construct a convex hull around them (rubber band around the centroid points)
...
#Combine the centroids into one feature and draw voronoi polygons (rain gauges)
