...
```{r, data.summary, message=FALSE, warning=FALSE, include=FALSE}
...
```{r, data.summary kable, echo=FALSE, message=FALSE, warning=FALSE}
#Use the `kable` function to produce the 2 tables in markdown format

knitr::kable(summary_PM25,caption = "Mean Particulates (2.5mm)", col.names = c("County", "$\\mu g/m^3$"), digits = 2)

knitr::kable(summary_Ozone,caption = "Mean Ozone", col.names = c("County", "$\\mu g/m^3$"), digits = 2)
...
Ozone_2018 <- epa_data %>%
  select("Month","Year", "Ozone") %>%
  filter(Year == 2018) %>%
  ggplot(aes(x= factor(Month,levels = 1:12, labels = month.abb)))+
  geom_boxplot(aes(y= Ozone))+
 labs( title = "Monthly Ozone Concentrations (2018)",
      x= "Month",
      y = expression(paste('Concentration ', (µg/m^3))))

#when I did y = expression(paste('Concentration ', mu*g/m^3))) and knitted the document, it changed the mu to a different symbol so I had to look up the 

plot(Ozone_2018)
```

```{r echo=FALSE, fig.align='left', fig.cap="2019 Monthly Ozone Levels", fig.height = 3, message=FALSE, warning=FALSE}
...
Ozone_2019 <- epa_data %>%
  ...
  ggplot(aes(x= factor(Month,levels = 1:12, labels = month.abb)))+
  geom_boxplot(aes(y= Ozone))+
  labs( title = "Monthly Ozone Concentrations (2019)",
      x= "Month",
      y = expression(paste('Concentration ', (µg/m^3))))
